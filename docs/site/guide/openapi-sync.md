...
4. **호환성 확인** — 기존 호출이나 응답 처리를 깨뜨릴 수 있는 변경을 `BREAKING`으로 표시합니다.
5. **충돌 해결** — 현재 값 유지 또는 명세 값 반영을 선택합니다.
6. **선택 반영** — 체크한 엔드포인트만 반영합니다. 삭제 후보는 기본 미선택입니다.
7. **저장과 복원** — 저장 성공 후 변경을 적용하며 직전 반영 상태를 복원할 수 있습니다.
