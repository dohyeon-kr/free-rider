...
- 새 요청은 HTTP / SSE / WebSocket 타입을 선택합니다. HTTP는 Response Inspector, SSE는 Event Stream, WebSocket은 Message Timeline으로 전환됩니다.
- HTTP는 Params / Headers / Body / Auth / Vars / Tests / Docs를 사용합니다. SSE/WS는 프로토콜에 맞는 Headers/Auth/Protocols/Settings와 실시간 대시보드를 제공합니다.
- WebSocket은 handshake 커스텀 헤더와 Bearer/Basic Auth, subprotocol, 세션 쿠키, 저장 메시지(JSON/Text)를 지원합니다.
- 단일 실행은 현재 편집본을 사용합니다. 컬렉션 실행은 저장된 HTTP 요청만 사용합니다.
...
‘엔드포인트 추가’로 저장된 HTTP 요청을 선택합니다. 실행 목록에서 제외해도 원본 요청은 남습니다. 순서를 변경하고 선택한 요청을 실행합니다. 실패 시 중단, 수동 중지, 요청별 결과 확인을 지원합니다. SSE/WebSocket은 종료 시점이 정해진 단일 응답 요청이 아니므로 현재 Runner에서는 제외합니다. 실행 목록 설정은 저장 버튼으로 보관합니다.
